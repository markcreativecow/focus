(function($) {
    $.jqmCalendar = function(element, options) {
        ...
        function init() {
            ...
            $("<a href='#' data-role='button' data-icon='carat-l' data-iconpos='notext' class='previous-btn'>Previous</a>").click(function() {
            ...
            $("<a href='#' data-role='button' data-icon='carat-r' data-iconpos='notext' class='next-btn'>Next</a>").click(function() {
                refresh(new Date(plugin.settings.date.getFullYear(), plugin.settings.date.getMonth() + 1, plugin.settings.date.getDate()));
            }).appendTo($th);
            ...
            $listview = $("<ul data-role='listview' data-inset='true' />").insertAfter($table);
        ...
        $element.bind('change', function(originalEvent, begin) {
            ...
            plugin.settings.eventHandler.getEventsOnDay(begin, end, function(list_of_events) {
                for(var i = 0, event; event = list_of_events[i]; i++ ) {
                    if (i == 0) {
                        $("<li data-role='list-divider'>" + event[plugin.settings.fullDate] + "</li>").appendTo($listview);                        
                    }
        ...
        function listItemFormatter($listItem, timeString, summary, event,bg) {
            ...
            if (event[plugin.settings.url]) {
                $('<a></a>').html( text ).attr( 'href', event[plugin.settings.url] ).attr( 'class', 'view-conference-button' ).attr( 'data-id', event[plugin.settings.dataId] ).appendTo($listItem);
            } else {
                $listItem.text( text );
            }
